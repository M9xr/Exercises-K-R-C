...
struct tnode {		/* the tree node: */
	...
	int line_no;		/* what line the word occurs */
...
void treeprint(struct tnode *p) {
	...
	if (p != NULL) {
		treeprint(p->left);
		/* If a particular word appear more frequently than 1 time in 200 entires, then it won't be printed */
		if ((val = ((double)(p->count / (double)number_of_words) * 100)) < 0.5) {
